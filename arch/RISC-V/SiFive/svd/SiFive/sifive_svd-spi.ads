...
--  of SCK.
   ...
   type SCKMODE_Register is record
      --  Serial Clock Phase
      PHA           : SCKMODE_CPHA := SiFive_SVD.SPI.Val_0;
      --  Serial Clock Polarity
      POL           : SCKMODE_CPOL := SiFive_SVD.SPI.Val_0;
      --  unspecified
   ...
   type CSMODE_Register is record
      MODE          : CSMODE_Chip_Select_Modes := SiFive_SVD.SPI.Auto;
      --  unspecified
      Reserved_2_31 : HAL.UInt30 := 16#0#;
   end record
     with Volatile_Full_Access, Object_Size => 32,
...
--  protocol, the DQ0 pin is driven with the transmit data as normal.
   ...
   type FMT_Register is record
      PROTO          : FMT_SPI_Protocol := SiFive_SVD.SPI.Single;
      ENDIAN         : FMT_SPI_Endianness := SiFive_SVD.SPI.Msb_First;
      DIR            : FMT_SPI_IO_Direction := SiFive_SVD.SPI.Rx;
      --  unspecified
