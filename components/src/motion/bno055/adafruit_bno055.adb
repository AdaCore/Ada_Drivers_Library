...
package body Adafruit_BNO055 is

   procedure Delay_Milliseconds (Count : Positive);

   function Value_At (MSB, LSB : Register_Address) return Unsigned_16 with Inline;
   --  MSB and LSB are the *register addresses* from which to get the values,
   --  not the values themselves

   function Value_At (MSB, LSB : Register_Address) return Integer_16 with Inline;
   --  MSB and LSB are the *register addresses* from which to get the values,
   --  not the values themselves

   function To_Integer_16 (LSB, MSB : Unsigned_8) return Integer_16 with Inline;
   ...
   procedure Reset (This : BNO055_9DOF_IMU) is
      pragma Unreferenced (This);
      Reset_Bit : constant Unsigned_8 := 16#20#;
   begin
      Write (BNO055_SYS_TRIGGER_ADDR, Value => Reset_Bit);  -- x3F, x20
   ...
   is
   begin
      Set_Mode (This, Operating_Mode_Config);  -- x3D, x0

      --  select register map page zero
      Write (BNO055_PAGE_ID_ADDR, 0);    -- x7, x0
      Delay_Milliseconds (10);

      Write (BNO055_PWR_MODE_ADDR, Power_Mode'Enum_Rep);   --  x3E, x0 (for normal power)
      Delay_Milliseconds (10);

      --  clear interrupt, self-test, and reset bits, conditionally set
      --  external oscillator bit
      if Use_External_Crystal then
         Write (BNO055_SYS_TRIGGER_ADDR, Value => 16#80#);
      else
         Write (BNO055_SYS_TRIGGER_ADDR, Value => 0);
      end if;
      Delay_Milliseconds (10);

      Set_Mode (This, Operating_Mode);  -- x3D, x0C (for Operating_Mode_NDOF)
   ...
   function Selected_Euler_Units
     (This  : BNO055_9DOF_IMU)
      return Euler_Angle_Units
   is
      pragma Unreferenced (This);
      Value : Unsigned_8;
   begin
      Read (BNO055_UNIT_SEL_ADDR, Value);
   ...
   function Selected_Acceleration_Units (This  : BNO055_9DOF_IMU)
   ...
   function Selected_Temperature_Units (This  : BNO055_9DOF_IMU)
   ...
   is
      ...
      Result : Unsigned_8;
   begin
      Read (BNO055_CHIP_ID_ADDR, Value => Result);
      return Result;
   end Device_Id;
   ...
   begin
      if This.Mode = Operating_Mode_Config then
         Write (BNO055_OPR_MODE_ADDR, Value => Mode'Enum_Rep);
         Delay_Milliseconds (Switching_From_Config_Mode);
      else
         Write (BNO055_OPR_MODE_ADDR, Value => Operating_Mode_Config'Enum_Rep);
   ...
   function Current_Mode (This : BNO055_9DOF_IMU) return Operating_Modes is
      Result : Unsigned_8 := 0;
      pragma Unreferenced (This);
   begin
      Write (BNO055_PAGE_ID_ADDR, 0);    -- x7, x0
      ...
      Read (BNO055_OPR_MODE_ADDR, Value => Result);
   ...
   is
      pragma Unreferenced (This);
   begin
      Read (BNO055_ACCEL_REV_ID_ADDR, Info.Accelerometer);
      Read (BNO055_MAG_REV_ID_ADDR, Info.Magnetometer);
      Read (BNO055_GYRO_REV_ID_ADDR, Info.Gyroscope);
      Read (BNO055_BL_REV_ID_ADDR, Info.Bootloader);
      Info.Software := Value_At (MSB => BNO055_SW_REV_ID_MSB_ADDR, LSB => BNO055_SW_REV_ID_LSB_ADDR);
   ...
   is
      ...
      Value : Unsigned_8;
   begin
      Read (BNO055_SYS_STAT_ADDR, Value);
      System_Status := System_Status_Values'Val (Value);
      Read (BNO055_SELFTEST_RESULT_ADDR, Value);
      Self_Test := As_Self_Test_Results (Value);
      Read (BNO055_SYS_ERR_ADDR, Value);
   ...
   function Sensor_Calibration (This : BNO055_9DOF_IMU) return Calibration_States is
      pragma Unreferenced (This);
      Data   : Unsigned_8;
      Mask   : constant := 2#00000011#;
      Result : Calibration_States;
   begin
      Read (BNO055_CALIB_STAT_ADDR, Data);
   ...
   begin
      ...
      Result (X) := Real (New_X) / LSB;
      Result (Y) := Real (New_Y) / LSB;
      Result (Z) := Real (New_Z) / LSB;
   ...
   function Quaternion_Orientation (This : BNO055_9DOF_IMU)
      return Quaternion
   is
      pragma Unreferenced (This);
      Buffer : Byte_Buffer (0 .. 7);
      New_W  : Integer_16;
      New_X  : Integer_16;
      New_Y  : Integer_16;
      New_Z  : Integer_16;
      Result : Quaternion;
      Scale  : constant Real := 1.0 / Real (2 ** 14); -- see section 3.6.5.5
   begin
      Read_Buffer (BNO055_QUATERNION_DATA_W_LSB_ADDR, Buffer);
      --  By reading multiple bytes, the device ensures data consistency,
      --  whereas reading single bytes individually does not have that
      --  guarantee. See the Datasheet, section 3.7 "Data register shadowing"
      ...
      Result (1) := Real (New_W) * Scale;
      Result (2) := Real (New_X) * Scale;
      Result (3) := Real (New_Y) * Scale;
      Result (4) := Real (New_Z) * Scale;
   ...
   begin
      ...
      Write (BNO055_TEMP_SOURCE_ADDR, Source'Enum_Rep);
      Read (BNO055_TEMP_ADDR, Value => Result);
   ...
   begin
      ...
      Offsets.Accel_Offset_X := Value_At (MSB => ACCEL_OFFSET_X_MSB_ADDR, LSB => ACCEL_OFFSET_X_LSB_ADDR);
      Offsets.Accel_Offset_Y := Value_At (MSB => ACCEL_OFFSET_Y_MSB_ADDR, LSB => ACCEL_OFFSET_Y_LSB_ADDR);
      Offsets.Accel_Offset_Z := Value_At (MSB => ACCEL_OFFSET_Z_MSB_ADDR, LSB => ACCEL_OFFSET_Z_LSB_ADDR);

      Offsets.Gyro_Offset_X := Value_At (MSB => GYRO_OFFSET_X_MSB_ADDR, LSB => GYRO_OFFSET_X_LSB_ADDR);
      Offsets.Gyro_Offset_Y := Value_At (MSB => GYRO_OFFSET_Y_MSB_ADDR, LSB => GYRO_OFFSET_Y_LSB_ADDR);
      Offsets.Gyro_Offset_Z := Value_At (MSB => GYRO_OFFSET_Z_MSB_ADDR, LSB => GYRO_OFFSET_Z_LSB_ADDR);

      Offsets.Mag_Offset_X := Value_At (MSB => MAG_OFFSET_X_MSB_ADDR, LSB => MAG_OFFSET_X_LSB_ADDR);
      Offsets.Mag_Offset_Y := Value_At (MSB => MAG_OFFSET_Y_MSB_ADDR, LSB => MAG_OFFSET_Y_LSB_ADDR);
      Offsets.Mag_Offset_Z := Value_At (MSB => MAG_OFFSET_Z_MSB_ADDR, LSB => MAG_OFFSET_Z_LSB_ADDR);

      Offsets.Accel_Radius := Value_At (MSB => ACCEL_RADIUS_MSB_ADDR, LSB => ACCEL_RADIUS_LSB_ADDR);
      Offsets.Mag_Radius := Value_At (MSB => MAG_RADIUS_MSB_ADDR, LSB => MAG_RADIUS_LSB_ADDR);
   ...
   begin
      ...
      Write (ACCEL_OFFSET_X_LSB_ADDR, Value => Unsigned_8 (Outgoing and 16#FF#));
      Write (ACCEL_OFFSET_X_MSB_ADDR, Value => Unsigned_8 (Shift_Right (Outgoing, 8)));

      Outgoing := Unsigned_16'Mod (Offsets.Accel_Offset_Y);
      Write (ACCEL_OFFSET_Y_LSB_ADDR, Value => Unsigned_8 (Outgoing and 16#FF#));
      Write (ACCEL_OFFSET_Y_MSB_ADDR, Value => Unsigned_8 (Shift_Right (Outgoing, 8)));

      Outgoing := Unsigned_16'Mod (Offsets.Accel_Offset_Z);
      Write (ACCEL_OFFSET_Z_LSB_ADDR, Value => Unsigned_8 (Outgoing and 16#FF#));
      Write (ACCEL_OFFSET_Z_MSB_ADDR, Value => Unsigned_8 (Shift_Right (Outgoing, 8)));
      ...
      Write (GYRO_OFFSET_X_LSB_ADDR, Value => Unsigned_8 (Outgoing and 16#FF#));
      Write (GYRO_OFFSET_X_MSB_ADDR, Value => Unsigned_8 (Shift_Right (Outgoing, 8)));

      Outgoing := Unsigned_16'Mod (Offsets.Gyro_Offset_Y);
      Write (GYRO_OFFSET_Y_LSB_ADDR, Value => Unsigned_8 (Outgoing and 16#FF#));
      Write (GYRO_OFFSET_Y_MSB_ADDR, Value => Unsigned_8 (Shift_Right (Outgoing, 8)));

      Outgoing := Unsigned_16'Mod (Offsets.Gyro_Offset_Z);
      Write (GYRO_OFFSET_Z_LSB_ADDR, Value => Unsigned_8 (Outgoing and 16#FF#));
      Write (GYRO_OFFSET_Z_MSB_ADDR, Value => Unsigned_8 (Shift_Right (Outgoing, 8)));
      --  The Datasheet, section 3.6.4.3 "Gyroscope offset" says the
      --  configuration only occurs when the MSB of the Z offset is written

      Outgoing := Unsigned_16'Mod (Offsets.Mag_Offset_X);
      Write (MAG_OFFSET_X_LSB_ADDR, Value => Unsigned_8 (Outgoing and 16#FF#));
      Write (MAG_OFFSET_X_MSB_ADDR, Value => Unsigned_8 (Shift_Right (Outgoing, 8)));

      Outgoing := Unsigned_16'Mod (Offsets.Mag_Offset_Y);
      Write (MAG_OFFSET_Y_LSB_ADDR, Value => Unsigned_8 (Outgoing and 16#FF#));
      Write (MAG_OFFSET_Y_MSB_ADDR, Value => Unsigned_8 (Shift_Right (Outgoing, 8)));

      Outgoing := Unsigned_16'Mod (Offsets.Mag_Offset_Z);
      Write (MAG_OFFSET_Z_LSB_ADDR, Value => Unsigned_8 (Outgoing and 16#FF#));
      Write (MAG_OFFSET_Z_MSB_ADDR, Value => Unsigned_8 (Shift_Right (Outgoing, 8)));
      --  The Datasheet, section 3.6.4.2 "Magnetometer offset" says the
      --  configuration only occurs when the MSB of the Z offset is written

      Outgoing := Unsigned_16'Mod (Offsets.Accel_Radius);
      Write (ACCEL_RADIUS_LSB_ADDR, Value => Unsigned_8 (Outgoing and 16#FF#));
      Write (ACCEL_RADIUS_MSB_ADDR, Value => Unsigned_8 (Shift_Right (Outgoing, 8)));
      ...
      Write (MAG_RADIUS_LSB_ADDR, Value => Unsigned_8 (Outgoing and 16#FF#));
      Write (MAG_RADIUS_MSB_ADDR, Value => Unsigned_8 (Shift_Right (Outgoing, 8)));
      --  The Datasheet, section 3.6.4.4 "Radius" says the
      --  configuration only occurs when the MSB of the radius is written
   ...
   begin
      ...
      Write (BNO055_UNIT_SEL_ADDR, Value);
      Delay_Milliseconds (10);
   ...
   begin
      ...
      Write (BNO055_AXIS_MAP_CONFIG_ADDR, Value => New_Mapping);
      Delay_Milliseconds (10);
   end Remap_Axes;
   ...
   begin
      ...
      Write (BNO055_AXIS_MAP_SIGN_ADDR, Value => New_Mapping);
      Delay_Milliseconds (10);
   ...
   function Value_At (MSB, LSB : Register_Address) return Integer_16 is
      High, Low : Unsigned_8;
   begin
      Read (Register => MSB, Value => High);
      Read (Register => LSB, Value => Low);
   ...
   function Value_At (MSB, LSB : Register_Address) return Unsigned_16 is
      High, Low : Unsigned_8;
      Result    : Unsigned_16;
   begin
      Read (Register => MSB, Value => High);
   ...
   function To_Integer_16 (LSB : Unsigned_8;  MSB : Unsigned_8) return Integer_16 is
      Result : Integer_32;
   begin
