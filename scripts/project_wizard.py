...
def middleware_config(config):
    ...
    if (runtime_profile.startswith("light-tasking")
       or runtime_profile.startswith("embedded")):
...
def ADL_configuration(config, project_directory, project_name,
   ...
   Build_Checks : Disabled_Or_Enabled_Type := \
external ("ADL_BUILD_CHECKS", "Disabled");
...
description = \
    """Welcome to the Ada Drivers Library (ADL) project wizard. This script
    will ask you some questions to define the ADL configuration of your
    project. It will then generate the different files required to use ADL
    based on this configuration."""

parser = argparse.ArgumentParser(description=description)

parser.add_argument('definitions', nargs='*',
                    help="""Pre defined config key=value. Use this to give
                    predefined answer to the configuration question. For
                    instant Board=STM32F407_Discovery""")

parser.add_argument('-i', '--input-config', type=argparse.FileType('r'),
                    action='append',
                    help="""Load pre defined config keys from a file. The file
                    must containt couples of key=value, only one per line and
                    nothing else.""")

parser.add_argument('--script-mode', action="store_true",
                    help="""Enable script mode. In script mode there is no
                    input required from the user. Configuration values will be
                    taken from pre-defined values (--input-config or command
                    line definitions), or default value if any. The
                    configuration will fail if there are no pre-defined or
                    default value for a key. This mode can be used to
                    automatically generated ADL project file from a known
                    configuration.""")

parser.add_argument('--dont-use-defaults', action="store_true",
                    help="""When this switch is used in script mode, the
                    configuration will not take into account default values.
                    This means that the configuration will fail if a key
                    doesn't have a pre-defined value from --input-config or
                    command line definitions. This can be used to automatically
                    detect discrepency when upgrading to a new version of ADL,
                    for instance if a new config key was added.""")


parser.add_argument("-d", "--project-dir",
                    help="""Directory where the project will be created
                    (default is Ada_Drivers_Library). Use '.' to create the
                    project in the current directory.""",
                    default="Ada_Drivers_Library")

parser.add_argument("-p", "--project-name",
                    help="""Name of the project file that will be generated
                    (default is Ada_Drivers_Library)""",
                    default="Ada_Drivers_Library")

parser.add_argument("-s", "--source-dir",
                    help="""Name of the source directory for you ADL project
                    (default is src). This is where the Ada confiuration file
                    will be written. You can also use this directory to add
                    you own package specification and/or package implementation
                    to the ADL project. The source directory will be created
                    inside the project directory""",
                    default="src")

parser.add_argument("-o", "--object-dir",
                    help="""Name of the object output directory that will be
                    used for your ADL project (default is obj). The object
